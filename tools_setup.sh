...
echo "╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝   ╚══════╝╚══════╝   ╚═╝    ╚═════╝ ╚═╝     "
echo -e "${NC}"
echo -e "${CYAN}Enhanced Recon Tools Installation for Parrot OS (ZSH)${NC}"
echo ""
...
if [[ $EUID -eq 0 ]]; then
   ...
   print_warning "Consider running as regular user for better Go tool management"
else
...
# Ensure Go directories exist (since you already have Go configured)
mkdir -p $HOME/go/bin
mkdir -p $HOME/go/src
